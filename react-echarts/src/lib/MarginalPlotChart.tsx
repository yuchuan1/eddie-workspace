...
const MarginalPlotChart: React.FC<MarginalPlotProps> = ({
  ...
  const option: EChartsOption = useMemo(() => {
    ...
    const makeHistogram = (values: number[], bc = binCount) => {
      ...
      const densities = counts.map((c) => (normalize ? c / (Math.max(1, n) * width) : c));
    ...
    return {
      title: { text: chartData.title || 'Marginal Plot', top: '0%', left: 'center' },
      grid: [
        { left: '10%', right: '25%', top: '28%', bottom: '15%', containLabel: true },
        { left: '10%', right: '25%', top: '10%', bottom: '78%', containLabel: true },
      ...
      xAxis: [
        ...
        { type: 'category', gridIndex: 1, data: xHist.labels, axisLabel: { show: showMarginalLabels }, axisTick: { show: false }, axisLine: { show: false }, splitLine: { show: false } },
        { type: 'value', gridIndex: 2, show: false },
      ],
      yAxis: [
        { type: 'value', gridIndex: 0, min: yMin, max: yMax, splitLine: { show: true } },
