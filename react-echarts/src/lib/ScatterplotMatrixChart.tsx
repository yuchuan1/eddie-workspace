...
const ScatterplotMatrixChart: React.FC<{ chartData: ChartData; theme?: 'light' | 'dark' }> = ({ chartData, theme = 'light' }) => {
    ...
    for (let i = 0; i < gridSize; i++) {
      for (let j = 0; j < gridSize; j++) {
        ...
        if (i === j) {
          ...
          const binLabels = Array.from({ length: bins }, (_, idx) =>
            `${(min + idx * binSize).toFixed(1)}-${(min + (idx + 1) * binSize).toFixed(1)}`
          );
          ...
          option = {
            ...
            xAxis: {
              ...
              data: binLabels,
              axisLabel: { rotate: 45 },
              ...(i === 0 ? { name: chartData.axisLabels.x[i], nameLocation: 'middle', nameGap: 30 } : {}),
            },
            yAxis: { type: 'value', ...(j === 0 ? { name: chartData.axisLabels.y[i], nameLocation: 'middle', nameRotate: 90, nameGap: 45 } : {}) },
            series: [{
              type: 'bar',
              data: histogram,
              barWidth: '80%',
          ...
          const scatterData = chartData.series.find(s =>
            s.name === `${chartData.axisLabels.y[i]}_${chartData.axisLabels.x[j]}`
          )?.data || [];
          ...
          option = {
            ...
            xAxis: { type: 'value', ...(i === 0 ? { name: chartData.axisLabels.x[j], nameLocation: 'middle', nameGap: 30 } : {}) },
            yAxis: { type: 'value', ...(j === 0 ? { name: chartData.axisLabels.y[i], nameLocation: 'middle', nameRotate: 90, nameGap: 45 } : {}) },
            series: [{
              type: 'scatter',
              data: scatterData,
